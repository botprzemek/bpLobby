...
public class Fly implements CommandExecutor, TabCompleter {
    ...
    String setOn = plugin.getConfig().getString("messages.fly.set.on");
    String setOff = plugin.getConfig().getString("messages.fly.set.off");
    String setSpeed = plugin.getConfig().getString("messages.fly.set.speed");
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 2) {
            ...
            if (args[0].length() == 1) {
                ...
                target.sendMessage(IridiumColorAPI.process(prefix + setOn.replace("%player%", player.getName()).replace("%speed%", args[0])));
                target.playSound(player.getLocation(), Sound.valueOf(sound), 1.0f, 1.0f);
                target.setAllowFlight(true);
                ...
                target.sendMessage(IridiumColorAPI.process(prefix + setOff.replace("%player%", player.getName()).replace("%speed%", args[0])));
                target.playSound(player.getLocation(), Sound.valueOf(sound), 1.0f, 1.0f);
